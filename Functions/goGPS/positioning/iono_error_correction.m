...
% -------------------------------------------------------------------------
	...
	f = 1 + 16*(0.53-el).^3;%(step.6)
	
	psi = (0.0137 ./ (el+0.11)) - 0.022;%(step.1)
	
	phi = lat + psi .* cos(az*pi);%(step.2)
	phi(phi > 0.416)  =  0.416;
	phi(phi < -0.416) = -0.416;
	
	lambda = lon + ((psi.*sin(az*pi)) ./ cos(phi*pi));%(step.3)
	
	ro = phi + 0.064*cos((lambda-1.617)*pi);%(step.4)
	
	t = lambda*43200 + time_rx;
	t = mod(t,86400);%(step.5)
